...
const SKILL_GROUPS = ['ads', 'engineering-product', 'launch', 'marketing', 'operations', 'partner', 'pmf', 'sales', 'scale', 'strategy', 'support'];
...
Install options:
  ...
  --phase, -p   all | ads | engineering-product | launch | marketing | operations | partner | pmf | sales | scale | strategy | support
...
function ensurePhase(phase) {
  ...
  if (!SKILL_GROUPS.includes(phase)) {
    throw new Error(
      `Unknown phase '${phase}'. Valid phases: all, ${SKILL_GROUPS.join(', ')}`,
    );
  }
  return phase;
}

function listSkillDirs(phase = 'all', host = 'pi') {
  const skillsRoot = path.join(packageRoot, 'generated', host);
  if (!fs.existsSync(skillsRoot)) {
    throw new Error(`Missing generated/${host}. Run npm run os:gen:all before packaging.`);
  }

  const groupsToScan = phase === 'all' ? SKILL_GROUPS : [phase];
  const out = [];

  for (const groupName of groupsToScan) {
    const groupDir = path.join(skillsRoot, groupName);
    if (!fs.existsSync(groupDir)) continue;
    out.push(...findSkillDirs(groupDir));
  }
...
1. Read the generated SKILL.md file for that skill
...
`;
  ...
  for (const skillDir of skillDirs) {
    ...
    body += `**SKILL.md path:** generated/codex/${phase}/${skillName}/SKILL.md\n\n`;
  }
...
function resolveInstallArgs(options, positionals) {
  ...
  for (const token of tokens) {
    ...
    if (token === 'all' || SKILL_GROUPS.includes(token)) {
      config.phase = token;
      continue;
    }
...
function runInstall(options, positionals) {
  ...
  if (config.agent === 'all') {
    const piSkillDirs = listSkillDirs(config.phase, 'pi');
    const codexSkillDirs = listSkillDirs(config.phase, 'codex');
    if (piSkillDirs.length === 0 || codexSkillDirs.length === 0) {
      throw new Error(`No skills found for phase '${config.phase}'`);
    }
    installPi(piSkillDirs);
    installCodexSkills(codexSkillDirs);
    console.log('');
    console.log('Installed current generated skills for both pi and Codex.');
    return;
  }

  const skillDirs = listSkillDirs(config.phase, config.agent);
...
function checkSkillFile(label, filePath) {
  ...
  if (!text.includes('Human-First Response') && !text.includes('Human-facing response (required)')) {
    return { ok: false, message: `✗ ${label}: installed co-founder is missing the human-facing response section; reinstall` };
  }
...
function runList(options) {
  ...
  for (const phaseName of SKILL_GROUPS) {
